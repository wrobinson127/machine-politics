...
def fmt_evidence_date(e):
    """Render an evidence date honouring optional date_precision, so an
    unestablished day never renders as a fact. Values: 'day' (default) keeps
    the ISO date; 'month' renders 'November 2024'; 'year' renders '2024'.
    A malformed date falls back to the raw string unchanged."""
    raw = iso(e.get("date"))
    prec = e.get("date_precision", "day")
    if prec == "day" or not raw:
        return raw
    parts = raw.split("-")
    try:
        if prec == "month" and len(parts) >= 2:
            return f"{_MONTHS[int(parts[1])]} {parts[0]}"
        if prec == "year" and parts:
            return parts[0]
...
def waffles(iso3, name, entry, votes):
    ...
    legend = (
        '<div class="vlegend">'
        '<span class="vg" data-hi="v:y" data-info="<b>In favour.</b> Highlights the states that voted yes."><span class="cell y"></span> In favour</span>'
        '<span class="vg" data-hi="v:n" data-info="<b>Against.</b> Highlights the states that voted no."><span class="cell n"></span> Against</span>'
        ...
        for e in coding.get("evidence") or []:
            ...
            if d and key not in seen:
                ...
                acts.append((0, "paper", d,
                             ...
                             f"({esc(fmt_evidence_date(e))}).", e.get("url")))
    if not acts:
...
:root{--mp-g2:#F1ECE2;--mp-faint:#8A8479;--mp-rs:#B9B0A0;
  --mp-lbiban:#3B5BA5;--mp-lbiopen:#2E7F86;--mp-regsoft:#A9741F;
...
def _evi(e, sources):
    ...
    cite = f'{esc(src.get("title", e.get("source", "")))}, {esc(fmt_evidence_date(e))}'
